...
| **2e: Default paper-method abliteration** | Install refusal-direction hooks at worker startup by default so every Phase 2 candidate runs under the Macar §3.3 protocol that gave Phase 1.5 a 3.6× multiplier | ✅ Shipped 2026-04-24. `./scripts/start_worker.sh` defaults to paper-method; `VANILLA=1` opts out. UI tags each leaderboard card as `abliterated` or `vanilla`. See [ADR-017](decisions.md). |
