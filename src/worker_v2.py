"""Four-phase worker (the new local-only autoresearch loop).
...
State machine:
  ...
  │   - call directed_capraro.generate_candidates (or other strategy)     │
...
worker uses a HandleRegistry that enforces this on every transition.
...
from src.evaluate import CandidateSpec, load_eval_sets
...
def _install_signal_handlers() -> None:
    def handler(signum, _frame):
        ...
        _shutdown = True
        print(
            f"[worker_v2] received signal {signum}, shutting down at next safe point",
            flush=True,
        )
...
def _phase_a_one(
    ...
    try:
        ...
        print(f"[worker_v2] phase_a FAILED {spec.id}: {e}\n{tb}", flush=True)
...
def _phase_b_drain(
    ...
    if orphan_ids:
        print(
            f"[worker_v2] phase B picking up {len(orphan_ids)} orphan "
    ...
    for cid in candidate_ids + orphan_ids:
        try:
            ...
            tb = traceback.format_exc()
            print(f"[worker_v2] phase_b FAILED {cid}: {e}\n{tb}", flush=True)
...
def _phase_c_propose(
    ...
    for spec in specs:
        ...
        n_written += 1
    print(f"[worker_v2] phase C wrote {n_written} new candidates to queue/pending",
          flush=True)
    return n_written
...
def main_loop(
    *,
    ...
    if orphan_count > 0:
        print(
            f"[worker_v2] STARTUP crash-recovery: {orphan_count} orphan "
    ...
    while not _shutdown:
        if max_cycles and cycles >= max_cycles:
            print(f"[worker_v2] hit max_cycles={max_cycles}, exiting", flush=True)
            break
        ...
        if pending_left < propose_threshold:
            ...
            try:
                ...
                tb = traceback.format_exc()
                print(f"[worker_v2] phase_c FAILED: {e}\n{tb}", flush=True)
        else:
            if items:
    ...
    print(
        f"[worker_v2] shutdown. cycles={cycles}. DB summary: {summary}",
        flush=True,
    )
    registry.unload_all()
    return cycles
...
def main() -> int:
    ...
    print("[worker_v2] starting four-phase local-only worker", flush=True)
    ...
    print(f"[worker_v2] eval set: {len(held_out)} held-out, "
